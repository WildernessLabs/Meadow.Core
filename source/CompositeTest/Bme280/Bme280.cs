...
namespace Meadow.Foundation.Sensors.Atmospheric
{
    ...
    /// </remarks>
    public class Bme280 :
        FilterableChangeObservable<CompositeChangeResult<Pressure, Temperature, RelativeHumidity>, Pressure, Temperature, RelativeHumidity>,
        ITemperatureSensor, IHumiditySensor, IBarometricPressureSensor
    {
        ...
        public (Pressure Pressure, Temperature Temperature, RelativeHumidity Humidity) Conditions;

        /// <summary>
        /// </summary>
        public event EventHandler<CompositeChangeResult<Pressure, Temperature, RelativeHumidity>> Updated = delegate { };
        public event EventHandler<CompositeChangeResult<Temperature>> TemperatureUpdated = delegate { };
        ...
        public async Task<(Pressure Pressure, Temperature Temperature, RelativeHumidity Humidity)> Read(
            ...
            int standbyDuration = 1000)
        {
            ...
            lock (_lock) 
            {
                ...
                (Pressure Pressure, Temperature Temperature, RelativeHumidity Humidity) oldConditions;

                CompositeChangeResult<Pressure, Temperature, RelativeHumidity> result;
                ...
                Task.Factory.StartNew(async () => {
                    while (true) {
                        ...
                        // capture history
                        oldConditions = (Conditions.Pressure, Conditions.Temperature, Conditions.Humidity);

                        // read
                        await Read(temperatureSampleCount, pressureSampleCount, humiditySampleCount);

                        // build a new result with the old and new conditions
                        result = new CompositeChangeResult<Pressure, Temperature, RelativeHumidity>(oldConditions, Conditions);
        ...
        protected void RaiseChangedAndNotify(CompositeChangeResult<Pressure, Temperature, RelativeHumidity> changeResult)
        {
        ...
        /// </remarks>
        protected async Task<(Pressure Pressure, Temperature Temperature, RelativeHumidity Humidity)> Update()
        {
            return await Task.Run(() => 
            {
                (Pressure Pressure, Temperature Temperature, RelativeHumidity Humidity) conditions;
                ...
                //
                conditions.Temperature = new Temperature((float)(((tfine * 5) + 128) >> 8) / 100);
                //
